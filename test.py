...
video_path = 'file/upload_video/12.avi'
ball_cuts = ball_cutframe(video_path, 960)
...
ReCollect_Ball2Line = []
VideoFrameCount_Buf = []
for k, v in ball_frames.items():
    # print(VideoFrameCount_Buf)
    if len(VideoFrameCount_Buf) == 0:
        VideoFrameCount_Buf.append(k)
    else:
        # print(VideoFrameCount_Buf[-1] - k)
        if k - VideoFrameCount_Buf[-1] == int(960 / 240):
            VideoFrameCount_Buf.append(k)
            if len(VideoFrameCount_Buf) == 5:
                temp = ball_frames[VideoFrameCount_Buf[0]]
                for j in range(1, 5):
                    img = ball_frames[VideoFrameCount_Buf[j]]
